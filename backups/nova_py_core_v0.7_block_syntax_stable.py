...
def find_block_end(lines, start):
    ...
    for i in range(start, len(lines)):
        ...
        if not line or line.startswith("#"):
            continue
        if line.startswith("repeat ") and " then " not in line:
            depth += 1
...
def execute_lines(lines, vars_map, start=0, end=None):
    if end is None:
        end = len(lines)
    ...
    while i < end:
        ...
        if line.startswith("repeat "):
            ...
            for n in range(count):
                vars_map["i"] = str(n + 1)
                execute_lines(lines, vars_map, i + 1, block_end)
...
def help_menu():
    ...
    print("  NOVA file commands: say, note, set, calc, vars, if/end block, repeat/end block, identity, status, memory, history, backup")
